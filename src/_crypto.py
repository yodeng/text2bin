...
class CryptoData(object):
    ...
    @staticmethod
    def encrypt_data(data, key_loop=20):
    ...
    @classmethod
    def encrypt_file(cls, in_file=None, out_file=None, chunksize=64*1024, key_loop=50):
        key = "DFFa/1v5+RJMZnuqa5h5wdpvKoF9EEVy"
        iv = Random.new().read(AES.block_size)
        for i in range(key_loop):
            key = real_key(key)
    ...
    @classmethod
    def decrypt_data(cls, data, key_loop=20):
        key = "DFFa/1v5+RJMZnuqa5h5wdpvKoF9EEVy"
    ...
    @classmethod
    def decrypt_file_iter(cls, in_file, chunksize=64*1024, key_loop=50):
        key = "DFFa/1v5+RJMZnuqa5h5wdpvKoF9EEVy"
    ...
    @classmethod
    def decrypt_file(cls, in_file, out_file, chunksize=64*1024):
        with open(out_file, "wb") as fo:
            for data in cls.decrypt_file_iter(in_file, chunksize=chunksize):
                fo.write(data)
